...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("  - `doc` (built-in): .doc (Word 97-2003) read / edit — `doc read <f.doc>` · `doc replace <f.doc> \"old\" \"new\" [-o out.doc]` (in-place by default).")
    appendLine("    Wraps Spire.Doc with two proot fixes: auto `setarch -R` (the engine randomly aborts without it) + auto removal of the free-build evaluation notice after each save.")
