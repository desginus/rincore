...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("    `pdf <f.pdf>` renders PDF pages to PNG the same way · `officecli view <f> html` also available.")
    appendLine("  - EXTRACT text only with a concrete purpose (search / exact quoting / locating an edit):")
    appendLine("    `pdf <f.pdf> --text` (embedded text layer) · `officecli view <f> text` · `markitdown <file>` (docx/xlsx/pptx/pdf/html -> markdown tables).")
    appendLine("    Scanned documents: render and VIEW the pages (`pdf <f.pdf>`) — do not default to OCR; `pdf <f.pdf> --ocr` exists only for explicit text needs.")
