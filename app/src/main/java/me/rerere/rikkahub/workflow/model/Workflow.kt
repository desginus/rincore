...
data class WorkflowDefinition(
    ...
    /**
     * 创建/更新时捕获的可用工具名快照 (固定集合, 运行时只能使用此集合内的工具 —
     * 防止作者删除工具后旧工作流仍调用). 空 = 不限制.
     */
...
/** 工具名快照 (简单版 — 不依赖 fork 的能力目录体系) */
object WorkflowCapabilitySnapshot {
    fun capture(actions: List<WorkflowAction>): Set<String> = actions.map { it.tool }.toSortedSet()
    fun parse(set: Set<String>): Set<String> = set
}
