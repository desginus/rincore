/* 【域 G·自动化】 — 工作流 | 地图: docs/APP_MAP.md §G */
package me.rerere.rikkahub.workflow.execution


/* ───【自研】WorkflowEngine.kt — 原版无此文件
...
class WorkflowEngine(
    ...
    private suspend fun fireLocked(workflowId: String): FireOutcome {
        ...
        // Headless 执行上下文 — 与 CronJobWorker direct-mode 同一模式
...
class WorkflowActionRunner(
    private val directRunner: DirectModeActionRunner,
) {
    ...
    suspend fun run(
        ...
        val direct = actions.map { DirectModeActionRunner.Action(it.tool, it.args) }
        val result = directRunner.run(direct, availableTools)
        return if (result.finalOutcome == "success") {
            RunResult(true, null, summarize(actions))
        } else {
            RunResult(false, result.errorMessage, "")
        }
    }

    private fun summarize(actions: List<WorkflowAction>): String =
