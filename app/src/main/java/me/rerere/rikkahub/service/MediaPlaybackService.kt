 ...
 * Surfaces system media controls on lock screen, notification shade, Bluetooth headsets.
 * Start via [buildPlayIntent] or ACTION_* constants.
 */
class MediaPlaybackService : Service() {

    companion object {
        const val CHANNEL_ID = "rincore_media_playback"
    ...
    private fun initMediaSession() {
        ...
        mediaSession = MediaSessionCompat(this, "RinCoreMediaSession").apply {
    ...
    private fun startPlayback(
        source: String, title: String?, artist: String?, album: String?, artworkUri: String?,
        startPositionMs: Long = 0L,
    ) {
        releaseMediaPlayer()
        requestAudioFocus()
        ...
        if (resolvedTitle == null || resolvedArtist == null || resolvedAlbum == null) {
            try {
                MediaMetadataRetriever().use { retriever ->
                    ...
                    if (resolvedTitle == null) resolvedTitle = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_TITLE)
                    if (resolvedArtist == null) resolvedArtist = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_ARTIST)
                    if (resolvedAlbum == null) resolvedAlbum = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_ALBUM)
                }
    ...
    private fun stopPlayback() {
        ...
        if (!src.isNullOrBlank()) {
            lastStoppedSnapshot = StoppedSnapshot(
                source = src, title = currentTitle, artist = currentArtist,
                album = currentAlbum, artworkUri = currentArtworkUri,
    ...
    private fun requestAudioFocus(): Boolean {
        val focusListener = AudioManager.OnAudioFocusChangeListener { focusChange ->
            when (focusChange) {
                AudioManager.AUDIOFOCUS_LOSS, AudioManager.AUDIOFOCUS_LOSS_TRANSIENT -> pausePlayback()
    ...
    private fun buildState(state: Int, positionMs: Long, actions: Long = PlaybackStateCompat.ACTION_PLAY) =
    ...
    private fun postForegroundNotification() {
        ...
        val stopAction = NotificationCompat.Action(android.R.drawable.ic_delete, "Stop",
    ...
    private val mediaSessionCallback = object : MediaSessionCompat.Callback() {
        ...
        override fun onSkipToNext() = stopPlayback()
