...
class RouteActivity : ComponentActivity() {
    ...
    private fun ShareHandler(backStack: MutableList<NavKey>) {
        ...
        LaunchedEffect(Unit) {
            ...
            // singleTask 模式下，Activity 重建时 backStack 可能已有活跃的 ChatPage
            // 此时不应 clear，而是找到当前活跃对话并追加文件
            ...
            when (action) {
                Intent.ACTION_SEND -> {
                    ...
                    if (existingChatScreen != null && lastId == existingChatScreen.id) {
                        // Bug #3 修复: 当前对话窗口已存在，直接导航到它并附加文件
                        // 不需要 clear，Navigation3 会处理
