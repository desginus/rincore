...
## D19. v4.0.0 全量重写工程（2026-09-05）
- **背景**: v3.x 渐进修补累积的条件分支与历史补丁堆叠使逻辑走向晦涩，GenerationHandler 135 行嵌套 catch、CC API/ClaudeProvider 家族分支交错
- **决策**: 放弃渐进修补，四模块一次性结构级重写（RetryPolicy 抽象/WatchdogPolicy 跨族统一/请求构造分层/流解析收口），行为等价约束下数值参数与用户可见文案逐字保留
- **结果**: 感知层零差异，运维层可读性质变；后续 v4.0.4/5/6 的修复直接作用于干净结构，验证了重写收益
- **原则**: 用户定版"程序执行不能出现不知道走哪条路磨蹭半天"；"打补丁不如重写"自此成为铁律（v4.0.4/5/6 整段重写延续）

## D20. 工具图片兼容四方案迭代（v4.0.3→v4.0.6）
- **迭代路径**: 内嵌 tool_result content（拒）→ 同 user 消息混排（拒）→ 拆独立 user 消息（连续 user 拒）→ 序列化层拆分+全序列规范化后处理（终态）
- **核心决策**: 不猜协议转换层，逐形状实证；qwen 兼容层按 shape 校验，唯一安全形状集合四条（B118）；拆分动作与规范化动作必须分离为两个阶段（拆分在序列化点，规范在序列化完成后全序列扫描）
- **附带**: 极简错误体可读化 + innerNonText 嵌套统计（排障基建）；CC 图片兼容从 opt-in 转换器演进为序列化层默认剥离（转换器保留幂等）
...
6. 勿重新启用 limitContext 滞回策略——历史上启用即破坏缓存前缀（v3.3.5 回滚，v3.3.12 确认）
