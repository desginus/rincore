...
## D7 修正. 工具注入方向（v3.5.19，用户决策，铁律）
- **铁律**：工具分层注入是底层逻辑，禁止再改（用户明确指示）
- **数量注入**：模型知晓工具池全貌（总数 + 域分布，layer1 静态注入），工具定义仍按域分层加载
- **教训**：3.5.18-beta2 全量注入 264 工具 → 请求体 13→264 → 冷启动 100K+（回归）；已回滚
- **正确组合**：分层动态注入（请求体小）+ MCP 声明静态化（工具池由配置决定 → 数量稳定可注入 system）+ skill 直注
- **缓存阶梯化**（9.7K→30K→90K）根因为 MCP 连接状态波动删工具 → tools 变化 → 前缀断；MCP 静态化修复，非客户端其他因素
